...
export const Route = createFileRoute("/instagram-signup")({
  component: InstagramSignupCallback,
});

function InstagramSignupCallback() {
      ...
      <div className="bg-white p-8 rounded-2xl shadow-xl max-w-md w-full text-center space-y-6">
        ...
        <div className="space-y-2">
          ...
          <p className="text-slate-500">
            {status === "processing" && "Aguarde enquanto vinculamos o seu Instagram. Não feche esta janela."}
            {status === "success" && "Sua conta do Instagram foi vinculada com sucesso. Você já pode fechar esta janela."}
            {status === "error" && (search.error_description || "Não foi possível validar o código de acesso da Meta.")}
          </p>
        </div>

        <div className="pt-4 flex justify-center">
          {status === "processing" && <Loader2 className="h-8 w-8 text-pink-600 animate-spin" />}
