      ...
      {isLoadingUnits ? (
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {units.map((unit) => (
            <Card
              key={unit.id}
              className="overflow-hidden flex flex-col group transition-all hover:shadow-md border-border/60"
            >
              <div className="h-1.5 w-full" style={{ backgroundColor: unit.color || "#6366f1" }} />
              <CardHeader className="pb-3 flex flex-row justify-between items-start">
                <div>
                  <CardTitle className="flex items-center gap-2 text-xl">{unit.name}</CardTitle>
                  <CardDescription className="font-mono text-xs mt-1">{unit.slug}</CardDescription>
                </div>
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button
                      variant="ghost"
                      size="icon"
                      className="-mr-2 -mt-2 h-8 w-8 text-muted-foreground opacity-50 group-hover:opacity-100 transition-opacity"
                    >
              ...
              <CardFooter className="pt-0 border-t border-border/40 mt-auto bg-muted/10 px-0">
                <Button
                  variant="ghost"
                  className="w-full rounded-none h-12 justify-between px-6 hover:bg-muted/30"
        ...
        <div className="rounded-lg border border-dashed p-12 text-center text-muted-foreground bg-card/30">
          ...
          <p className="text-sm mt-1">
            Crie sua primeira unidade acima para começar a configurar instâncias de WhatsApp.
          </p>
        </div>
      )}
      ...
      <Dialog open={!!editingUnit} onOpenChange={(v) => !v && setEditingUnit(null)}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Atualize o nome e a cor de identificação da unidade.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-2">
              ...
              <div className="flex items-center gap-3">
                <div className="relative">
                  <Input
                    ...
                    className="w-10 h-10 p-0 border-0 rounded-md cursor-pointer overflow-hidden shadow-sm"
                    value={editUnitColor}
                    onChange={(e) => setEditUnitColor(e.target.value)}
                  />
                  <div className="absolute inset-0 ring-1 ring-inset ring-black/10 rounded-md pointer-events-none" />
                </div>
            ...
            <div className="space-y-2">
              ...
              <Input
                value={editUnitDocument}
                onChange={(e) => setEditUnitDocument(e.target.value)}
            ...
            <div className="space-y-2">
              ...
              <Input
                value={editUnitBusinessHours}
                onChange={(e) => setEditUnitBusinessHours(e.target.value)}
            ...
            <div className="pt-2 border-t mt-2">
                ...
                {editUnitCustomVars.map((v, i) => (
                  <div key={i} className="flex gap-2 items-center">
                    ...
                    <Button
                      ...
                      onClick={() =>
                        setEditUnitCustomVars(editUnitCustomVars.filter((_, idx) => idx !== i))
                      }
                    >
                      X
                    </Button>
              ...
              <Button
                ...
                onClick={() =>
                  setEditUnitCustomVars([...editUnitCustomVars, { key: "", value: "" }])
                }
              >
                + Adicionar Variável
              </Button>
          ...
          <DialogFooter>
            ...
            <Button
              onClick={() => {
                const customVarsObj = editUnitCustomVars.reduce(
                  (acc, curr) => {
      ...
      <AlertDialog open={!!deletingUnit} onOpenChange={(v) => !v && setDeletingUnit(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Tem certeza que deseja excluir a unidade <strong>{deletingUnit?.name}</strong>? Essa
              ação não pode ser desfeita e falhará se existirem usuários vinculados a ela.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
...
function UnitManagementSheet({
  ...
  const [confirmDialog, setConfirmDialog] = useState<{
    open: boolean;
    type: "disconnect" | "delete" | "delete_dept" | null;
    instance: any;
    dept: any;
  }>({ open: false, type: null, instance: null, dept: null });
    ...
    try {
      ...
      try {
        ...
        const evoInst = evogoInstances.find(
          (e: any) =>
            e.token === instance.evogo_api_key ||
            e.instance?.apikey === instance.evogo_api_key ||
            e.instance?.instanceName === instance.instance_name,
        );
        if (evoInst) {
          const rawOwner = evoInst.jid || evoInst.instance?.owner;
          if (rawOwner) {
            ownerJid = rawOwner.split("@")[0].split(":")[0];
          }
          if (
            evoInst.state ||
            evoInst.instance?.state ||
            evoInst.status ||
            evoInst.instance?.status
          ) {
            evoInstState =
              evoInst.state ||
              evoInst.instance?.state ||
  ...
  const createDept = useMutation({
    mutationFn: async (name: string) => {
      const { error } = await supabase
        .from("departments")
        .insert({ company_id: company.id, unit_id: unit.id, name });
      if (error) throw error;
    },
    onSuccess: () => {
    ...
    onError: (e) =>
      toast.error("Erro ao criar departamento", { description: (e as Error).message }),
  });
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="sm:max-w-[600px] w-full overflow-y-auto p-0 flex flex-col h-full bg-background">
          ...
          <Tabs defaultValue="instances" className="w-full mt-4">
            <TabsList className="grid w-full grid-cols-2 mb-6">
              ...
              <TabsTrigger value="departments" className="flex items-center gap-2">
                <Layers className="h-4 w-4" /> Departamentos
              </TabsTrigger>
            </TabsList>
              ...
              <div className="flex items-center justify-between bg-card border border-border rounded-lg p-4 shadow-sm">
                <div className="space-y-0.5">
                  ...
                  <p className="text-xs text-muted-foreground font-normal">
                    Adicione instâncias do WhatsApp (EvoGo ou Oficial), Instagram ou Facebook
                    Messenger.
                  </p>
                </div>
                ...
                {isLoadingInstances ? (
                  ...
                  <div className="flex flex-col gap-3">
                    {instances.map((inst) => (
                      <div
                        key={inst.id}
                        className="group flex items-center justify-between rounded-lg border border-border/60 bg-card p-3 hover:border-border transition-colors"
                      >
                        <div>
                          <div className="flex items-center gap-2 mb-1">
                            <span className="font-semibold text-sm">{inst.name}</span>
                            {inst.provider &&
                              inst.provider !== "whatsapp" &&
                              inst.provider !== "evogo" && (
                                <Badge
                                  variant="outline"
                                  className={`text-[9px] py-0 px-1.5 h-4 capitalize font-semibold ${
                                    inst.provider === "instagram"
                                      ? "border-pink-200 text-pink-700 bg-pink-50/50"
                              ...
                              <Badge
                                ...
                                className={
                                  inst.status === "connected"
                                    ? "bg-success text-[10px] py-0 h-4 font-semibold"
                                    : "text-[10px] py-0 h-4 font-semibold"
                                }
                              >
                          ...
                          {inst.owner_jid && (
                            <p className="text-[11px] font-medium text-foreground/80 font-mono mt-1 flex items-center gap-1">
                              <Smartphone className="h-3 w-3" />+{inst.owner_jid}
                            </p>
                          )}
                        </div>
                        <div className="flex gap-1.5 opacity-100 sm:opacity-50 group-hover:opacity-100 transition-opacity">
                          {inst.provider === "oficial" ? (
                            <Badge
                              variant="outline"
                              className="h-8 px-2.5 border-emerald-200 text-emerald-700 bg-emerald-50 text-[11px] font-medium flex items-center gap-1"
                            >
                              <svg
                            ...
                            <Badge
                              variant="outline"
                              className="h-8 px-2.5 border-pink-200 text-pink-700 bg-pink-50 text-[11px] font-medium flex items-center gap-1"
                            >
                              <svg
                                xmlns="http://www.w3.org/2000/svg"
                                width="12"
                            ...
                            <Badge
                              variant="outline"
                              className="h-8 px-2.5 border-blue-200 text-blue-700 bg-blue-50 text-[11px] font-medium flex items-center gap-1"
                            >
                              <svg
                                xmlns="http://www.w3.org/2000/svg"
                                width="12"
                            ...
                            <>
                              ...
                              >
                                <RefreshCw
                                  className={`h-4 w-4 text-primary ${refreshingInstance === inst.id ? "animate-spin" : ""}`}
                                />
                              </Button>
                              {inst.status === "connected" ? (
                                <Button
                                  variant="outline"
                                  size="sm"
                                  className="h-8 px-3 text-xs text-destructive hover:bg-destructive/10 hover:text-destructive border-destructive/20"
                                  onClick={() =>
                                    setConfirmDialog({
                                      open: true,
                                      type: "disconnect",
                    ...
                    {departments.map((dept) => (
                      ...
                      >
                        ...
                        <Button
                          variant="ghost"
                          size="icon"
                          className="h-7 w-7 text-muted-foreground hover:text-destructive hover:bg-destructive/10 opacity-100 sm:opacity-0 group-hover:opacity-100 transition-all"
                          onClick={() =>
                            setConfirmDialog({
                              open: true,
                              type: "delete_dept",
        ...
        <AlertDialog
          ...
          onOpenChange={(open) =>
            !open && setConfirmDialog({ open: false, type: null, instance: null, dept: null })
          }
        >
          <AlertDialogContent>
            <AlertDialogHeader>
            ...
            <AlertDialogFooter>
              ...
              <AlertDialogAction
                onClick={() => {
                  ...
                  else if (confirmDialog.type === "delete_dept")
                    handleDeleteDept(confirmDialog.dept);
                }}
                className="bg-destructive text-destructive-foreground hover:bg-destructive/90"
              >
