...
export function SalesCoachSheet({
  conversationId,
  open,
  onOpenChange,
  onSuggestion,
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="w-full sm:max-w-xl md:max-w-2xl p-0 flex flex-col border-l border-border bg-card">
        <SheetHeader className="p-5 border-b border-border bg-muted/20 shrink-0">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <div>
                <SheetTitle className="text-lg">Sales Coach IA</SheetTitle>
                <SheetDescription>
          ...
          {isLoadingAnalysis ? (
            ...
            <div className="space-y-6">
              ...
              <div className="prose prose-sm dark:prose-invert max-w-none 
                prose-table:w-full prose-table:border-collapse prose-table:text-sm prose-td:border prose-td:border-border prose-td:p-3 prose-th:border prose-th:border-border prose-th:bg-muted/50 prose-th:p-3 prose-th:text-left
        ...
        {latestAnalysis && (
          <div className="p-5 border-t border-border bg-card shrink-0 shadow-[0_-10px_20px_-10px_rgba(0,0,0,0.05)] space-y-4">
            ...
            {tacticalSuggestion && (
              <div className="bg-amber-500/10 border border-amber-500/20 rounded-xl p-4 text-sm space-y-3 relative overflow-hidden">
                ...
                <div className="prose prose-sm dark:prose-invert prose-p:my-1 prose-strong:text-amber-700 dark:prose-strong:text-amber-400">
                  <ReactMarkdown remarkPlugins={[remarkGfm]}>
                    {tacticalSuggestion}
                  </ReactMarkdown>
                </div>
