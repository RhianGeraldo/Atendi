...
export async function sendPlatformMessage({
  ...
  if (conv.channel === 'whatsapp') {
    ...
    if (!instance || !instance.oficial_phone_number_id || !instance.oficial_waba_id || !instance.oficial_access_token) {
      throw new Error("Instagram Account ID, Page ID or Token missing");
    }
    ...
    const response = await fetch(`https://graph.facebook.com/v20.0/${instance.oficial_waba_id}/messages?access_token=${instance.oficial_access_token}`, {
      method: 'POST',
