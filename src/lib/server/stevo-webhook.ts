...
export async function handleStevoWebhook(request: Request): Promise<Response> {
  try {
    ...
    await processStevoWebhookBody(body);
...
export async function processStevoWebhookBody(body: any): Promise<void> {
  try {
    ...
    console.log('Stevo Webhook received:', JSON.stringify(logBody, null, 2));
      ...
      if (instanceName && jid && jidAlt && jid.includes('@lid') && jidAlt.includes('@s.whatsapp.net')) {
        ...
        if (instance) {
          ...
          if (contact) {
            ...
            console.log(`[stevo-webhook] CTWA LID Unificado: LID ${lidNumber} -> Real ${realNumber}`);
          }
        }
      }
      return;
    ...
    // Handle message.upsert (Baileys/Evolution API format) or Message/SendMessage (WhatsMeow/Stevo format)
    if (body.event === 'messages.upsert' || body.event === 'messages.update' || body.event === 'Message' || body.event === 'SendMessage') {
      ...
      if (body.event === 'Message' || body.event === 'SendMessage') {
        ...
        if (msg.conversation) {
          ...
          // Stevo provides decrypted base64 - prefer it over thumbnail for full quality
          if (msg.base64) {
          ...
          if (msg.protocolMessage.type === 14 || msg.protocolMessage.type === 'MESSAGE_EDIT') {
            ...
            if (targetId) {
              console.log('[stevo-webhook] Revoking message ID:', targetId);
              const { error } = await supabaseAdmin
                .from('messages')
                .update({ is_deleted: true })
                .eq('remote_msg_id', targetId);
              if (error) console.error('[stevo-webhook] Error revoking message:', error);
            }
            return;
          } else {
            return;
          ...
          // Stevo sends edited messages as secretEncryptedMessage if it fails to decrypt them
          // We must ignore them to prevent "[Mídia/Mensagem não suportada]" spam
          console.log('[stevo-webhook] Ignoring secretEncryptedMessage/Edit without decrypted content');
          return;
          ...
          console.log('[stevo-webhook] Ignoring empty text message');
          return;
        }
      ...
      console.log(`[stevo-webhook] Lookup conversation: instance_id=${instance_id}, contact_id=${contactId}, result=${JSON.stringify(latestConvs)}`);
      ...
      if (latestConvs && latestConvs.length > 0) {
        ...
        if (conv.status === 'resolved') {
          console.log(`[stevo-webhook] Reopening resolved conversation: ${conversationId}`);
          ...
          if (existingSession) {
            ...
            if (sessionErr) {
              if (sessionErr.code === '23505') {
                ...
                console.error('[stevo-webhook] Error creating session:', sessionErr);
              }
            } else if (newSession) {
          ...
          console.log(`[stevo-webhook] Found active/waiting conversation: ${conversationId}`);
        }
        ...
        console.log(`[stevo-webhook] No existing conversation found. Creating new one.`);
        ...
        if (convErr) {
          ...
          console.warn('[stevo-webhook] Conversation insert failed, checking for race condition:', convErr.message);
          ...
          if (racedConv) {
            console.log('[stevo-webhook] Found conversation created by parallel call:', racedConv.id);
        ...
        try {
          ...
          if (sourceId && origThumb && origThumb.startsWith('http') && !origThumb.includes('supabase.co')) {
            ...
            if (existingAd && existingAd.thumbnail_url && existingAd.thumbnail_url.includes('supabase.co')) {
              ...
              console.log(`[stevo-webhook] Reusing cached ad thumbnail for sourceId: ${sourceId}`);
            } else {
              // Download and cache
              console.log(`[stevo-webhook] Downloading ad thumbnail for sourceId: ${sourceId}`);
              ...
              if (response.ok) {
                ...
                if (!uploadErr) {
                  ...
                  console.log(`[stevo-webhook] Successfully cached ad thumbnail to: ${publicUrlData.publicUrl}`);
                } else {
                  console.error('[stevo-webhook] Error uploading ad thumbnail:', uploadErr);
                }
              }
            }
          }
        } catch (e) {
          console.error('[stevo-webhook] Error caching ad thumbnail:', e);
        }
      }
      ...
      if (msgErr) {
        if (msgErr.code === '23505') {
          console.log('[stevo-webhook] Duplicate remote_msg_id during insert. Updating metadata if present:', remoteMsgId);
      ...
      if (Object.keys(metadata).length > 0 && metadata.externalAdReply && !isFromMe) {
        try {
          ...
          console.error('[stevo-webhook] Failed to register ad lead:', e);
        }
      }
      ...
      if (aiActive && newMessage && !isFromMe) {
        ...
        console.log(`[stevo-webhook] Queueing AI response for conversation ${conversationId}, message ${newMessage.id}`);
    ...
    // Return 200 to acknowledge receipt and prevent endless retries from Stevo
    return;
  }
}
