...
const OLD_DB_URL = 'postgresql://postgres.kcirbzbdahliqcwvjifo:Erriesse2025!@aws-1-us-west-2.pooler.supabase.com:5432/postgres';
const NEW_DB_URL = 'postgresql://postgres.ruxfdwtytywenujsqppq:Erriesse2025!@aws-0-ca-central-1.pooler.supabase.com:5432/postgres';
...
const contactMergedMappings: Array<{ id: string; merged_into_id: string }> = [];
  ...
  for (let i = 0; i < rows.length; i += BATCH_SIZE) {
    ...
    for (const row of chunk) {
      ...
      for (const col of columns) {
        ...
        else if (tableName === 'public.contacts' && col === 'merged_into_id') {
          if (row[col]) {
            contactMergedMappings.push({ id: row.id, merged_into_id: row[col] });
          }
          values.push(null);
        }
        else {
...
async function main() {
  ...
  try {
    ...
    await oldClient.query('SET statement_timeout = 0');
    await newClient.query('SET statement_timeout = 0');
    console.log('Conectado com sucesso (timeout desabilitado)!');
    ...
    execSync(
      'psql "postgresql://postgres.ruxfdwtytywenujsqppq:Erriesse2025!@aws-0-ca-central-1.pooler.supabase.com:5432/postgres" -f old_public_schema.sql',
      { stdio: 'inherit' }
    );
    ...
    // 6.5. Resolver dependência autorreferenciada de contacts.merged_into_id
    if (contactMergedMappings.length > 0) {
      console.log(`\n[6.5/6] Resolvendo dependências autorreferenciadas para ${contactMergedMappings.length} contatos...`);
      const BATCH_SIZE = 500;
      for (let i = 0; i < contactMergedMappings.length; i += BATCH_SIZE) {
        const chunk = contactMergedMappings.slice(i, i + BATCH_SIZE);
        const placeholders: string[] = [];
        const values: any[] = [];
        let paramIndex = 1;
        for (const item of chunk) {
          placeholders.push(`($${paramIndex++}::uuid, $${paramIndex++}::uuid)`);
          values.push(item.id, item.merged_into_id);
        }
        const updateQuery = `
          UPDATE public.contacts AS c
          SET merged_into_id = v.merged_into_id
          FROM (VALUES ${placeholders.join(', ')}) AS v(id, merged_into_id)
          WHERE c.id = v.id
        `;
        await newClient.query(updateQuery, values);
      }
      console.log('Dependências autorreferenciadas resolvidas (contacts).');
    }
